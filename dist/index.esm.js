...
import { StablePool, ZERO, WeightedMaths, WeightedPool, MetaStablePool, PhantomStablePool, LinearPool, SwapTypes, PoolFilter, SOR, parseToPoolsDict, getSpotPriceAfterSwapForPath } from '@balancer-labs/sor';
...
class StablePoolJoin {
    constructor() {
        ...
        this.buildJoin = ({ joiner, pool, tokensIn, amountsIn, slippage, wrappedNativeAsset, }) => {
            ...
            const expectedBPTOut = SDK.StableMath._calcBptOutGivenExactTokensIn(new OldBigNumber(parsedAmp), sortedBalances.map((b) => new OldBigNumber(b)), sortedAmounts.map((a) => new OldBigNumber(a)), new OldBigNumber(parsedTotalShares), new OldBigNumber(parsedSwapFee)).toString();
    ...
    constructor() {
        ...
        this.buildExitExactBPTIn = ({ exiter, pool, bptIn, slippage, shouldUnwrapNativeAsset, wrappedNativeAsset, singleTokenMaxOut, }) => {
            ...
            if (singleTokenMaxOut) {
                ...
                const amountOut = SDK.WeightedMath._calcTokenOutGivenExactBptIn(new OldBigNumber(sortedBalances[singleTokenMaxOutIndex]), new OldBigNumber(sortedWeights[singleTokenMaxOutIndex]), new OldBigNumber(bptIn), new OldBigNumber(parsedTotalShares), new OldBigNumber(parsedSwapFee)).toString();
                // Apply slippage
                ...
                const amountsOut = SDK.WeightedMath._calcTokensOutGivenExactBptIn(sortedBalances.map((b) => new OldBigNumber(b)), new OldBigNumber(bptIn), new OldBigNumber(parsedTotalShares)).map((amount) => amount.toString());
                // Apply slippage
        ...
        this.buildExitExactTokensOut = ({ exiter, pool, tokensOut, amountsOut, slippage, wrappedNativeAsset, }) => {
            ...
            const bptIn = SDK.WeightedMath._calcBptInGivenExactTokensOut(sortedBalances.map((b) => new OldBigNumber(b)), sortedWeights.map((w) => new OldBigNumber(w)), sortedAmounts.map((a) => new OldBigNumber(a)), new OldBigNumber(parsedTotalShares), new OldBigNumber(parsedSwapFee)).toString();
            // Apply slippage
...
class MetaStablePoolJoin {
    constructor() {
        ...
        this.buildJoin = ({ joiner, pool, tokensIn, amountsIn, slippage, wrappedNativeAsset, }) => {
            ...
            const expectedBPTOut = SDK.StableMath._calcBptOutGivenExactTokensIn(new OldBigNumber(parsedAmp), scaledBalances.map((b) => new OldBigNumber(b)), scaledAmounts.map((a) => new OldBigNumber(a)), new OldBigNumber(parsedTotalShares), new OldBigNumber(parsedSwapFee)).toString();
            const minBPTOut = subSlippage(BigNumber.from(expectedBPTOut), BigNumber.from(slippage)).toString();
            const userData = StablePoolEncoder.joinExactTokensInForBPTOut(sortedAmounts, minBPTOut);
            const to = balancerVault;
            const functionName = 'joinPool';
